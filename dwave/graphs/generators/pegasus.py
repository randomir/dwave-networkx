    ...
    def graph_to_linear(self, g):
        ...
        if labels == 'int':
            ...
            raise ValueError(
                ...
                "Input must be generated by dwave.graphs.pegasus_graph."
            )
...
def pegasus_sublattice_mappings(source, target, offset_list=None):
    ...
    if target.graph.get('family') != 'pegasus':
        raise ValueError("Source graph must be a Pegasus graph constructed by "
    ...
    if source.graph.get('family') == 'chimera':
        if source.graph['tile'] != 4:
            raise ValueError("Cannot construct sublattice mappings from Chimera "
                             "to Pegasus unless the Chimera tile parameter is 4")
        ...
        raise ValueError("source graph must be a Chimera graph or Pegasus graph "
                         "constructed by dwave.graphs.chimera_graph or "
                         "dwave.graphs.pegasus_graph respectively")
